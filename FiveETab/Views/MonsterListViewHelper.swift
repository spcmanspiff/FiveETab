...
//
//   let monsterDetails = try? newJSONDecoder().decode(MonsterDetails.self, from: jsonData)

import Foundation

// MARK: - MonsterDetails
struct MonsterDetails: Codable {
    let id, index, name, size: String?
    let type, subtype, alignment: String?
    ...
    let damageVulnerabilities, damageResistances: [JSONAny]?
    let damageImmunities: [String]?
    let conditionImmunities: [ConditionImmunity]?
    let senses: Senses?
...
// MARK: - Damage
struct Damage: Codable, Identifiable {
    let id = UUID()
    let damageType: ConditionImmunity?
...
// MARK: - ConditionImmunity
struct ConditionImmunity: Codable {
    let name, url: String?
}

// MARK: - Dc
struct Dc: Codable {
    let dcType: ConditionImmunity?
    let dcValue: Int?
